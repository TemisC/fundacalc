"""
Generador PDF — Módulo 9.2 · Muro de Gravedad.
"""
...
class GeneradorPDFMuroGravedad:
    ...
    def _generar_pdf(self, ruta, motor, norma):
        ...
        sty.add(ParagraphStyle('T2', parent=sty['Heading2'], fontSize=12,
                               ...
                               textColor=colors.HexColor(_AZUL)))
        sty.add(ParagraphStyle('N',  parent=sty['Normal'],
                               fontSize=9, leading=13))
        sty.add(ParagraphStyle('Nc', parent=sty['Normal'],
                               fontSize=9, leading=13, alignment=TA_CENTER))
        sty.add(ParagraphStyle('OK', parent=sty['Normal'],
                               fontSize=8, leading=12,
                               textColor=colors.HexColor(_VERDE)))
        sty.add(ParagraphStyle('ERR', parent=sty['Normal'],
                               fontSize=8, leading=12,
                               textColor=colors.HexColor(_ROJO)))
        sty.add(ParagraphStyle('ADV', parent=sty['Normal'],
                               fontSize=8, leading=12,
        ...
        # ── Portada ─────────────────────────────────────────────────────
        story.append(Paragraph("FundaCalc — Módulo 9.2", sty['T1']))
        story.append(Paragraph("Muro de Gravedad (H. Ciclópeo / Mampostería)", sty['T1']))
        ...
        story += self._tbl2([
            ...
            ("Ancho en la base",      f"{inp['b_base']:.2f} m"),
            ("Ancho en la corona",    f"{inp['b_corona']:.2f} m"),
            ("Enterramiento del pie", f"{inp['h_emb']:.2f} m"),
            ("Área sección",          f"{res.A_seccion:.3f} m²"),
        ], "Geometría")
        ...
        # ── Estabilidad ──────────────────────────────────────────────────
        e_r = res.estabilidad
        story.append(Paragraph("2. Verificación de Estabilidad", sty['T2']))
        story.append(Paragraph(
            f"Ka (Rankine) = {e_r.Ka:.4f} &nbsp;&nbsp; "
            f"Ea = {e_r.Ea:.2f} kN/m &nbsp;&nbsp; "
        ...
        # Tabla fuerzas
        story.append(Paragraph("Fuerzas estabilizantes:", sty['N']))
        fuerza_data = [
            ["Elemento",    "Peso (kN/m)",       "Brazo (m)",          "Momento (kN·m/m)"],
            ["Cuerpo muro", f"{e_r.W_muro:.2f}", f"{e_r.x_CG:.3f}",   f"{e_r.Mr:.2f}"],
            ["TOTAL",       f"{e_r.W_muro:.2f}", "—",                  f"{e_r.Mr:.2f}"],
        ]
        ...
        fs_data = [
            ...
            ["Vuelco",
             f"FS = {e_r.FS_vuelco:.2f}",
             "2.00",
             ok_str(e_r.ok_vuelco)],
            ["Deslizamiento",
             f"FS = {e_r.FS_desliz:.2f}",
             "1.50",
             ok_str(e_r.ok_desliz)],
            ["Presión base",
             f"q_max = {e_r.q_max:.1f} kPa",
             f"qa = {inp['qa']:.1f} kPa",
             ok_str(e_r.ok_presion)],
            ["Excentricidad",
             f"|e| = {abs(e_r.e):.3f} m",
             f"B/6 = {inp['b_base']/6:.3f} m",
             ok_str(e_r.ok_excentricidad)],
        ]
        ...
        story.append(Spacer(1, 0.2 * cm))
        story.append(Paragraph(
            f"Presión base: q_max = <b>{e_r.q_max:.1f} kPa</b> (toe) | "
            f"q_min = <b>{e_r.q_min:.1f} kPa</b> (heel) | "
            f"Empuje pasivo Ep = {e_r.Ep:.2f} kN/m",
            sty['N']))
        story.append(Paragraph(
            f"Posición resultante: x_R = {e_r.x_R:.3f} m desde toe | "
            f"Excentricidad e = {e_r.e:.4f} m",
            sty['N']))

        # ── Mensajes ─────────────────────────────────────────────────────
        story.append(Paragraph("3. Mensajes del Verificador", sty['T2']))
        ...
        story.append(Spacer(1, 0.5 * cm))
        story.append(Paragraph(
            "Generado por FundaCalc — Módulo 9.2 Muro de Gravedad",
            sty['Nc']))
    ...
    def _generar_txt(self, ruta, motor, norma):
        ...
        e   = res.estabilidad
        lines = [
            "FundaCalc — Módulo 9.2: Muro de Gravedad",
            f"Fecha: {datetime.now():%d/%m/%Y %H:%M}",
            "",
            f"H={res.H:.2f}m  b_base={res.b_base:.2f}m  b_corona={res.b_corona:.2f}m",
            f"Ka={res.Ka:.4f}  Ea={e.Ea:.2f}kN/m  Mo={e.Mo:.2f}kN·m/m",
            f"W={e.W_muro:.2f}kN/m  Mr={e.Mr:.2f}kN·m/m",
